...
const RideOfferCard = ({ ride, onBook }: { ride: any; onBook: () => void }) => (
    <motion.div
      key={ride.id}
      ...
      <Card className="hover:shadow-soft transition-all duration-300 cursor-pointer">
        <CardContent className="p-4">
          ...
          <div className="space-y-2">
            ...
            {ride.description && <p className="text-sm text-muted-foreground mt-2">{ride.description}</p>}
  
            <div className="pt-2">
              <Button className="w-full" onClick={onBook}>Book Ride</Button>
            </div>
          </div>
        </CardContent>
      </Card>
